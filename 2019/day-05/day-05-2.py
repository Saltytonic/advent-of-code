class IntcodeProcessor:
    ...
    def __jump_if_true_instruction(self,modes):
        if self.__read(self.__inst_ptr+1,modes[0]) != 0:
            self.__inst_ptr = self.__read(self.__inst_ptr+2,modes[1])
        else:
            self.__incr_pointer(3)

    def __jump_if_false_instruction(self,modes):
        if self.__read(self.__inst_ptr+1,modes[0]) == 0:
            self.__inst_ptr = self.__read(self.__inst_ptr+2,modes[1])
        else:
            self.__incr_pointer(3)

    def __less_than_instruction(self,modes):
        if self.__read(self.__inst_ptr+1,modes[0]) < self.__read(self.__inst_ptr+2,modes[1]):
            self.__write(self.__read(self.__inst_ptr+3,1),1)
        else:
            self.__write(self.__read(self.__inst_ptr+3,1),0)
        self.__incr_pointer(4)

    def __equals_instruction(self,modes):
        if self.__read(self.__inst_ptr+1,modes[0]) == self.__read(self.__inst_ptr+2,modes[1]):
            self.__write(self.__read(self.__inst_ptr+3,1),1)
        else:
            self.__write(self.__read(self.__inst_ptr+3,1),0)
    ...
    def __process_instruction(self):
        ...
        if opcode == 1:
            ...
            self.__jump_if_true_instruction(modes)
        elif opcode == 6:
            self.__jump_if_false_instruction(modes)
        elif opcode == 7:
            self.__less_than_instruction(modes)
        elif opcode == 8:
            self.__equals_instruction(modes)
        elif opcode == 99:
